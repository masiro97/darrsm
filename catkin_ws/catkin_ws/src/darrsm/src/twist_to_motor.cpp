...
// 	srv.request.right_wheel_velocity = 0;
// 	srv.request.left_wheel_velocity = 0;

// 	ros::NodeHandle n;

// 	ros::ServiceClient wheel_control_client_=
// 	n.serviceClient<dynamixel_workbench_msgs::SetDirection>("/dynamixel_workbench_tutorials/wheel");
...
void TwistToMotors::spin()
{
	...
	// signal(SIGINT, (void *)inter_handler);
...
void TwistToMotors::spinOnce()
{
	...
	right = (( 1.3 * dx ) + (dr * w /1.6))/wheel_radius;
	left = (( 1.3 * dx ) - (dr * w /1.6))/wheel_radius;
